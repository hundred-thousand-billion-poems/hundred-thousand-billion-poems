...
export function generateRandomPoemId(): string {
    const result = [];
    for (let i = 0; i < 10; i++) {
...
export function getPoemById(id: string): string[] {
    const result = [];
    for (let i = 0; i < 14; i++) {
        let index = Number(id[i]);
        if (!isFinite(index) || index < 0 || index > 9) {
            index = 1;
        }
        result.push(allLines[index][i]);
    }
    return result;
}
